...
export default function PainelModal(props){
  ...
  return (
      <>
          <ModalOverlay />
          <ModalContent bgImage={bgBody} borderRadius="20px"  border="1px solid gray" boxShadow='lg'>
              ...
              <Flex>
                  <Box w="30%"  p="4" className="flex flex-col">
                      {titles.map((element, i) => (
                          <Button key={i} leftIcon={icons[i]} onClick={() => handleClick(components[i], { name: name, index: index })} mb="2">{element}</Button>           
                      ))}
                  </Box>
